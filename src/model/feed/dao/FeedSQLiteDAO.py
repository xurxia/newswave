...
class FeedSQLiteDAO(FeedDAOInterface):
    ...
    def create_feed(self, feed : FeedDTO) -> int:
        try:
            sql : str = f'INSERT INTO SOURCE (NAME, URL, ETAG) VALUES (:name, :url, :etag)'
            params : dict = {'name': feed.name, 'url': feed.url, 'etag': ''}
    ...
    def get_feed(self, id : int) -> FeedDTO:
        try:
            sql : str = f'SELECT ID, NAME, URL, ETAG FROM SOURCE WHERE ID=:id'
            params : dict = {'id': id}
            cursor : Cursor = self._execute_sql(sql, params)
            row : Row = cursor.fetchone()
            feed : FeedDTO = None
            if row is not None:
                feed = FeedDTO(row[0], row[1], row[2], row[3])
        except ModelException as e:
    ...
    def get_feeds(self) -> list[FeedDTO]:
        try:
            sql : str = f'SELECT ID, NAME, URL, ETAG FROM SOURCE'
            cursor : Cursor = self._execute_sql(sql)
            rows : list[Row] = cursor.fetchall()
            feeds : list[FeedDTO] = []
            for row in rows:
                feed : FeedDTO = FeedDTO(row[0], row[1], row[2], row[3])
                feeds.append(feed)
        except ModelException as e:
    ...
    def update_feed(self, feed : FeedDTO) -> None:
        try:
            sql : str = f'UPDATE SOURCE SET NAME=:name, URL=:url, ETAG=:etag WHERE ID=:id'
            params : dict = {'id': feed.id, 'name': feed.name, 'url': feed.url, 'etag': feed.etag}
            cursor : Cursor = self._execute_sql(sql, params)
            if cursor.rowcount == 1:
                self._commit()
            else:
